...
* Fully offline translation between any pairing of 200 different languages.
* Uses Meta No Language Left Behind (NLLB) model under the hood. See [this link](https://huggingface.co/facebook/nllb-200-3.3B) for more details.
...
* *For some languages that do not use the latin alphabet*, displays transliteration for the translated output (For example, 한글 also shows *hangul* underneath).
  * I only implemented this for the languages that are "easy" to transliterate -- Greek, Russian, Korean, to name a few -- where the alphabet has a pretty straightforward letter-by-letter transliteration.

# How to Use

If you haven't already, you need to enable sideloading on your Supernote within `Settings->Security & Privacy`. This section requires an internet connection on your Supernote device, then forever after you can run translations offline.
...
3. Use your favorite sideloading application (I recommend [this one](https://github.com/nerdunit/androidsideloader/releases)) to install the app.
